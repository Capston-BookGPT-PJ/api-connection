...
public class SettingProfile extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        etUserId.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                // ✅ 수정: 화면에 @ 붙이기, null 또는 "null" 처리
                if (isUpdating) return;
                isUpdating = true;

                String text = s.toString();

                // ✅ 수정: 입력이 시작되면 앞에 @ 추가, 중복 방지
                if (!text.startsWith("@") && text.length() > 0) {
                    etUserId.setText("@" + text.replace("@", ""));
                    etUserId.setSelection(etUserId.getText().length()); // 커서 끝으로
                }
                // ✅ 커서를 항상 끝으로 이동
                etUserId.setSelection(etUserId.getText().length());
        ...
        getCall.enqueue(new Callback<UserResponse>() {
            ...
            public void onResponse(Call<UserResponse> call, Response<UserResponse> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    // ✅ 수정: tagId가 null이거나 "null"이면 화면에는 @만 표시
                    String tagId = currentUser.getTagId();
                    if (tagId == null || tagId.equals("null") || tagId.isEmpty()) {
                        etUserId.setText("@");
                    } else {
                        etUserId.setText("@" + tagId);
                    }
    ...
    private void saveProfileChanges() {
        ...
        if (tagIdStr.startsWith("@")) {
            tagIdStr = tagIdStr.substring(1); // ✅ 수정: 화면용 @ 제거
        }
        // ✅ 수정: 아무 값도 입력하지 않았으면 빈 문자열로 전송
        if (tagIdStr.isEmpty()) {
            tagIdStr = "";
        }

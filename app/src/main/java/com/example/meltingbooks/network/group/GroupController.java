...
public class GroupController {
    ...
    // GroupController.java 안에 추가
    // GroupController.java
    public void uploadGroupImage(int groupId, MultipartBody.Part file, OnGroupActionCallback callback) {
        SharedPreferences prefs = context.getSharedPreferences("auth", Context.MODE_PRIVATE);
        String token = prefs.getString("jwt", null);
        if (token == null || groupApi == null) {
            callback.onFailure("토큰 없음 또는 API 초기화 실패");
            return;
        }

        groupApi.uploadGroupImage("Bearer " + token, groupId, file)
                .enqueue(new Callback<ApiResponse<String>>() {
                    ...
                    public void onResponse(Call<ApiResponse<String>> call, Response<ApiResponse<String>> response) {
                        if (response.isSuccessful() && response.body() != null) {
                            ...
                            callback.onFailure("이미지 업로드 실패: 코드 " + response.code());
                        }
                    }
